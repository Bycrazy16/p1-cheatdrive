...
double** leggiMatrice(fstream& in, int r, int c) {
	...
	double** m = new double*[r];		//di larghezza righe
	for (i = 0; i < r; i++) {			// contatore < numeroDiRighe
		m[i] = new double[c];			// alloca m matrici monodimensionali di lunghezza colonne per il numero di righe
	}

	// Legge i dati dal file
	i = 0;								// riazzera lo stesso contatore per non sprecare memoria
	in >> d;							// inizia a leggere all'interno di d
	while (!in.eof()) {					// finché non finisci di leggere il file
		m[i / c][i % c] = d;			// scrivi d nelle celle della matrice bidimensionale
		i++;							// aumenta il contatore
		in >> d;						// continua a leggere all'interno di d
	}
